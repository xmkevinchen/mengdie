...
/// F-016 (AC3): the memory_ingest MCP output carries the two new additive
/// fields, and the pre-existing entry_id + conflicts fields are unchanged. An
/// unmatched resolve id is reported (not silently dropped) while the matching
/// id is superseded and the insert still succeeds.
#[tokio::test]
async fn ingest_reports_unmatched_resolves_via_mcp() {
    let h = Harness::new();

    // Seed a live predecessor.
    let first = h
        .ingest(IngestParams {
            title: "v1".to_string(),
            content: "first version of the auth decision".to_string(),
            source_file: "".to_string(),
            source_type: SourceType::Conclusion,
            knowledge_type: KnowledgeType::Decisional,
    ...
    // Additive contract: existing fields behave as before; new fields default.
    assert!(first.unmatched_resolves.is_empty());
    assert!(!first.conflict_scan_degraded);
    let id_a = first.entry_id.clone();

    // Re-ingest with one live resolve id + one bogus id.
    let second = h
        .ingest(IngestParams {
            title: "v2".to_string(),
            content: "second version of the auth decision".to_string(),
            source_file: "".to_string(),
            source_type: SourceType::Conclusion,
            knowledge_type: KnowledgeType::Decisional,
            entities: "auth".to_string(),
            project_id: None,
            resolves: Some(vec![id_a.clone(), "nonexistent-id".to_string()]),
        })
        .await;

    assert!(second.error.is_none(), "ingest errored: {:?}", second.error);
    assert!(
        !second.entry_id.is_empty(),
        "existing entry_id field intact"
    );
    assert_eq!(
        second.unmatched_resolves,
        vec!["nonexistent-id".to_string()],
        "bogus resolve id is reported, live id is not"
    );
    assert!(
        !second.conflict_scan_degraded,
        "healthy scan is not degraded"
    );

    // The live predecessor was superseded; the new memory is live.
    let old = h.db.get_memory(&id_a).unwrap().unwrap();
    assert!(old.valid_until.is_some(), "live resolve id was superseded");
}
