...
- **Ingestion**: MCP `memory_ingest` tool (primary, for AI agents) plus the
  `mengdie import` CLI for batch import of structured markdown artifacts
  (conclusion / review / plan files). A `notify`-based watcher library
  (`watcher.rs`) is written and unit-tested but NOT yet wired to any running
  daemon or CLI command — there is no live directory watch today.
