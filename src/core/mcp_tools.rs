...
pub struct IngestOutput {
    ...
    /// superseded. Additive — omitted from JSON when empty (the common case).
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub unmatched_resolves: Vec<String>,
    /// F-016 / BL-058: `true` when the conflict scan degraded — an empty
    /// `conflicts` list is then unreliable. Additive — omitted when false.
        ...
        match result {
            Ok(ingest_result) => {
                ...
                Json(IngestOutput {
                    ...
                    conflicts,
                    unmatched_resolves: ingest_result.unmatched_resolves,
                    conflict_scan_degraded: ingest_result.conflict_scan_degraded,
                    error: None,
                })
            }
            Err(e) => {
