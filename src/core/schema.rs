...
pub fn run_migrations(conn: &Connection) -> anyhow::Result<()> {
    ...
    // F-018 (BL-059): wrapped in an explicit transaction with the
    // `PRAGMA user_version` write INSIDE it (mirroring v5-v8), so the
    // multi-step ADD COLUMN → backfill → index swap is all-or-nothing and a
    // crash leaves either v1 or v2, never a half-migrated state stuck at
    // version 0.
    if current_version < 2 {
        ...
        let migration_result: anyhow::Result<()> = (|| {
            ...
            let rows: Vec<(String, String)> = stmt
                ...
                .filter_map(|r| r.ok())
                .collect();
            drop(stmt); // release the conn borrow before the index swap
    ...
    // F-018: the `column_exists` guard moves INSIDE the block so the
    // `user_version = 3` checkpoint still lands even if the column already
    // exists (a combined outer guard would skip the version write and leave
    // the span stuck).
    ...
    // F-018: wrapped — CREATE TABLE + 2 indexes is multi-step.
    ...
    // Migration v9 (F-017): exclude tombstoned rows from the content-hash
    // conflict so re-ingesting content matching a tombstoned row inserts a
    // fresh live row instead of refreshing + returning the dead row's id.
    //
    // The partial UNIQUE INDEX predicate and BOTH `ON CONFLICT(project_id,
    // content_hash) WHERE ...` clauses in db.rs MUST be byte-identical text,
    // same term order — SQLite matches the ON CONFLICT target to an index by
    // literal predicate, not logical equivalence (verified 3.51.0). The locked
    // predicate is: `WHERE source_type != 'synthesis' AND valid_until IS NULL`.
            ...
            conn.execute_batch(
                "DROP INDEX IF EXISTS idx_memory_content_hash;
                 CREATE UNIQUE INDEX idx_memory_content_hash
                     ON memory_entries(project_id, content_hash)
                     WHERE source_type != 'synthesis' AND valid_until IS NULL;",
            )?;
            // Recreate vec_memories_update with a tombstone guard in the WHEN
            // clause. An explicit DROP is required because the trigger was
            // installed via `CREATE TRIGGER IF NOT EXISTS` — without the DROP
            // the new WHEN clause would be silently skipped. Defense-in-depth:
            // under v9 the upsert no longer UPDATEs a tombstoned row, but any
            // other path that updates a tombstoned row's embedding must not
            // re-shadow it.
            conn.execute_batch(&format!(
                "DROP TRIGGER IF EXISTS vec_memories_update;
                 CREATE TRIGGER vec_memories_update
                     AFTER UPDATE OF embedding ON memory_entries
                     WHEN NEW.embedding IS NOT NULL AND NEW.embedding_dim = {VEC_DIM}
...
mod tests {
    ...
    // ---- F-018: v2/v3/v4 transactional migrations + user_version checkpoints ----

    /// AC1: a fresh migration reaches v9 with the v2/v3/v4 artifacts present.
    #[test]
    fn test_migration_chain_reaches_v9_with_v2_v3_v4_artifacts() {
        let conn = {
            crate::core::db::ensure_sqlite_vec_registered();
            Connection::open_in_memory().unwrap()
        };
        ...
        let version: i64 = conn
            ...
            .unwrap();
        assert_eq!(version, 9, "chain completes through the new v2-v4 wrappers");

        // v2: content_hash column + idx_memory_content_hash index.
        assert!(
            column_exists(&conn, "memory_entries", "content_hash").unwrap(),
            "v2 content_hash column present"
        );
        let idx: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master WHERE name = 'idx_memory_content_hash' AND type = 'index'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(idx, 1, "v2 idx_memory_content_hash present");
        // v3: invalidation_reason column.
        assert!(
            column_exists(&conn, "memory_entries", "invalidation_reason").unwrap(),
            "v3 invalidation_reason column present"
        );
        // v4: memory_synthesis_links table.
        let tbl: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master WHERE name = 'memory_synthesis_links' AND type = 'table'",
                [],
            ...
            .unwrap();
        assert_eq!(tbl, 1, "v4 memory_synthesis_links present");
    }

    /// AC2: faithful low-version resume. Surgically remove recoverable v2/v3/v4
    /// artifacts (the content_hash index + the synthesis table), reset
    /// user_version to 1, and re-migrate — the v2/v3/v4 blocks must ACTUALLY
    /// re-execute and rebuild them, reaching v9. (The naive "reset the integer
    /// only" form would skip every guarded statement and prove nothing.)
    /// DROP COLUMN content_hash is deliberately NOT used — it is fragile on an
    /// indexed column, and rebuilding the index + table already exercises the
    /// real low-version execution path; data-survival across migration is
    /// covered separately by `test_migration_from_v3_preserves_data`.
    #[test]
    fn test_migration_resumes_from_low_version_and_reexecutes() {
        let conn = {
            crate::core::db::ensure_sqlite_vec_registered();
            Connection::open_in_memory().unwrap()
        };
        ...
        // Tear down to a genuine pre-v2/pre-v4 state.
        conn.execute_batch(
            "DROP INDEX IF EXISTS idx_memory_content_hash;
             DROP TABLE IF EXISTS memory_synthesis_links;",
        )
        .unwrap();
        conn.execute_batch("PRAGMA user_version = 1;").unwrap();

        // Confirm the teardown actually removed them.
        let idx_before: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master WHERE name = 'idx_memory_content_hash'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(idx_before, 0, "teardown removed the v2 index");

        // Re-migrate: v2 must rebuild the index, v4 the table, reaching v9.
        run_migrations(&conn).unwrap();

        let version: i64 = conn
            .query_row("PRAGMA user_version", [], |r| r.get(0))
            .unwrap();
        assert_eq!(version, 9, "resume reaches v9");
        let idx_after: i64 = conn
            ...
            .unwrap();
        assert_eq!(idx_after, 1, "v2 re-executed and rebuilt the index");
        let tbl_after: i64 = conn
            .query_row(
            ...
            .unwrap();
        assert_eq!(tbl_after, 1, "v4 re-executed and rebuilt the table");
    }
    ...
    fn test_idx_memory_content_hash_is_partial_after_v5() {
        ...
        let sql: String = conn
            ...
            .unwrap();
        assert!(
            sql.contains("WHERE source_type != 'synthesis' AND valid_until IS NULL"),
            "expected v9 partial index excluding synthesis AND tombstoned rows, got: {sql}"
        );
    }
