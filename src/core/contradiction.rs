...
/// Result of a contradiction scan (F-016 / BL-058): the conflicts found, plus
/// a `degraded` flag set when the scan ran but could not complete cleanly —
/// e.g. a per-entity `facts_with_entity` lookup errored and was skipped. A
/// degraded scan returning zero conflicts is NOT the same as a clean scan that
/// genuinely found none; the caller surfaces the distinction. (A whole-call
/// failure surfaces as `Err` from `check_contradictions` and is mapped to
/// degraded by the caller.)
pub struct ConflictScanOutcome {
...
impl Db {
    ...
    /// Returns conflicts found + a degraded flag (caller decides what to do).
    pub fn check_contradictions(
        ...
        project_id: &str,
    ) -> anyhow::Result<ConflictScanOutcome> {
        if new_entities.is_empty() {
            return Ok(ConflictScanOutcome {
                conflicts: vec![],
                degraded: false,
            });
        }
        ...
        // F-016 / BL-058: a per-entity lookup that errors is skipped (scan
        // stays best-effort) but marks the whole scan degraded, so a partial
        // result is not mistaken for a clean "no conflicts".
