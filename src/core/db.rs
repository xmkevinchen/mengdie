...
/// Result of `insert_memory_resolving` (F-016 / BL-056): the inserted (or
/// content-hash-upserted) memory id, plus the deduped subset of `resolves`
/// ids that matched no live row in this project and were therefore NOT
/// superseded — reported, not silently dropped.
...
impl Db {
    ...
    /// On conflict with a LIVE row (same project_id + content_hash,
    /// `valid_until IS NULL`), updates metadata but preserves recall stats,
    /// timestamps, and ID (atomic via ON CONFLICT DO UPDATE). F-017: content
    /// matching a TOMBSTONED row (`valid_until IS NOT NULL`) is excluded from
    /// the conflict by the v9 partial index, so it inserts a fresh live row
    /// rather than refreshing + returning the dead id.
    ...
    fn insert_memory_inner(&self, id: String, mem: NewMemory) -> anyhow::Result<String> {
        ...
        let returned_id: String = tx.query_row(
            "INSERT INTO memory_entries
             ...
             ON CONFLICT(project_id, content_hash) WHERE source_type != 'synthesis' AND valid_until IS NULL DO UPDATE SET
    ...
    ///
    /// Best-effort + report (F-016 / BL-056): the new-memory insert always
    /// commits. Each `resolves` id whose supersession UPDATE matched no row —
    /// a typo, an id from another project (filtered by `AND project_id`), an
    /// already-tombstoned row, or the id this call itself just returned (the
    /// self-supersession guard) — is reported in `ResolveOutcome.unmatched`
    /// rather than silently dropped. Duplicate ids in `resolves` are processed
    /// once.
    pub fn insert_memory_resolving(
        ...
        // Best-effort + report: dedup, capture rows-affected, collect the
        // ids that matched nothing. The `id <> ?2` guard prevents a resolve id
        // equal to the upsert-returned id from invalidating the memory this
        // call just returned (data-corruption fix, codex review); the
        // `valid_until IS NULL` guard skips already-tombstoned rows. Either
        // makes the UPDATE match 0 rows, so the id surfaces as unmatched.
        let mut seen = std::collections::HashSet::new();
        let mut unmatched = Vec::new();
        for old_id in resolves {
            if !seen.insert(old_id.as_str()) {
                continue; // duplicate resolve id — already processed
            }
            let rows = tx.execute(
                "UPDATE memory_entries SET valid_until = ?1, superseded_by = ?2, invalidation_reason = 'superseded' \
                 WHERE id = ?3 AND project_id = ?4 AND id <> ?2 AND valid_until IS NULL",
                params![now, returned_id, old_id, project_id],
            )?;
            if rows == 0 {
                unmatched.push(old_id.clone());
            }
        }

        tx.commit()?;
        Ok(ResolveOutcome {
            entry_id: returned_id,
            unmatched,
        })
    }
    ...
    pub fn rename_project(&self, old_id: &str, new_id: &str) -> anyhow::Result<(usize, usize)> {
        ...
        let collisions: Vec<(String, String)> = {
            let mut stmt = tx.prepare(
                // F-017: only a live=live content_hash match is a genuine
                // collision (the post-rename partial-unique-index violation).
                // Under v9 a project may hold 1 live + N tombstoned rows per
                // hash; without these guards a live row masked by a tombstone
                // on the other side would be wrongly deleted (data loss).
                "SELECT old.id, old.title FROM memory_entries old
                 ...
                 WHERE old.project_id = ?1 AND new.project_id = ?2
                   AND old.valid_until IS NULL AND new.valid_until IS NULL",
            )?;
    ...
    pub fn rename_project_dry_run(
        ...
        let collision_count: i64 = conn.query_row(
            // F-017: live=live collisions only (see rename_project).
...
mod tests {
    ...
    // ---- F-017: re-ingest of content matching a tombstoned row ----

    fn mem_with_content(project_id: &str, content: &str) -> NewMemory {
        NewMemory {
            project_id: project_id.to_string(),
    ...
    /// AC1 (insert_memory path): re-ingesting content matching a tombstoned row
    /// inserts a fresh live row instead of returning the dead id.
    #[test]
    fn test_reingest_tombstoned_content_inserts_fresh() {
        let db = test_db();
        let id1 = db
            .insert_memory(mem_with_content("proj", "same content X"))
            .unwrap();
        assert!(db.invalidate_memory(&id1, None, Some("test")).unwrap());
        ...
        assert_ne!(
            id2, id1,
            "tombstone match must insert a fresh row, not return the dead id"
        );
        assert!(
            db.get_memory(&id2).unwrap().unwrap().valid_until.is_none(),
            "new row is live"
        );
        assert!(
            db.get_memory(&id1).unwrap().unwrap().valid_until.is_some(),
            "old row stays tombstoned"
        );

        let conn = db.conn.lock().unwrap();
        let n: i64 = conn
            .query_row(
                "SELECT count(*) FROM memory_entries WHERE content_hash = \
                 (SELECT content_hash FROM memory_entries WHERE id = ?1)",
                params![id2],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(n, 2, "1 live + 1 tombstoned share the content_hash");
    }

    /// AC1 (insert_memory_resolving path): same, with the old tombstoned id in
    /// `resolves` — it reports unmatched (F-016 `id <> ?2 AND valid_until IS NULL`).
    #[test]
    fn test_reingest_tombstoned_via_resolving() {
        let db = test_db();
        let id1 = db
            .insert_memory(mem_with_content("proj", "content Y"))
            .unwrap();
        assert!(db.invalidate_memory(&id1, None, Some("test")).unwrap());

        let outcome = db
            .insert_memory_resolving(
                mem_with_content("proj", "content Y"),
                std::slice::from_ref(&id1),
            )
            .unwrap();
        assert_ne!(
            outcome.entry_id, id1,
            "fresh row inserted via resolving path"
        );
        assert!(db
            .get_memory(&outcome.entry_id)
            .unwrap()
            .unwrap()
            .valid_until
            .is_none());
        assert_eq!(
            outcome.unmatched,
            vec![id1.clone()],
            "an already-tombstoned resolve id reports unmatched"
        );
    }

    /// AC2 / re-ingest chain: after the tombstone re-ingest, a third ingest of
    /// the same content dedups against the now-live row (tombstone doesn't fire).
    #[test]
    fn test_reingest_chain_dedups_on_new_live() {
        let db = test_db();
        let id1 = db
            .insert_memory(mem_with_content("proj", "content Z"))
            .unwrap();
        db.invalidate_memory(&id1, None, Some("test")).unwrap();
        let id2 = db
            .insert_memory(mem_with_content("proj", "content Z"))
            .unwrap();
        let id3 = db
            .insert_memory(mem_with_content("proj", "content Z"))
            .unwrap();
        assert_eq!(id3, id2, "3rd ingest dedups against the live row");
        assert_ne!(id2, id1);
    }

    /// AC4: rename must not delete a live row masked by a tombstone on the other side.
    #[test]
    fn test_rename_does_not_delete_live_masked_by_tombstone() {
        let db = test_db();
        let old_id = db
            .insert_memory(mem_with_content("old", "content W"))
            .unwrap();
        db.invalidate_memory(&old_id, None, Some("test")).unwrap();
        let new_id = db
            .insert_memory(mem_with_content("new", "content W"))
            .unwrap();

        let (_renamed, merged) = db.rename_project("old", "new").unwrap();
        assert_eq!(
            merged, 0,
            "a tombstoned old row is not a collision against the live new row"
        );
        assert!(
            db.get_memory(&new_id)
                .unwrap()
                .unwrap()
                .valid_until
                .is_none(),
            "the live new row must survive the rename"
        );
    }

    /// AC3: the vec_memories_update trigger guard — updating a tombstoned row's
    /// embedding writes no new vec0 shadow.
    #[test]
    fn test_trigger_guard_no_reshadow_on_tombstoned_update() {
        let db = test_db();
        let blob = vec![0u8; 384 * 4]; // 384 f32 zeros — valid vec0 payload
        let mut m = mem_with_content("proj", "content V");
        m.embedding = Some(blob.clone());
        m.embedding_dim = Some(384);
        let id = db.insert_memory(m).unwrap();
        db.invalidate_memory(&id, None, Some("test")).unwrap();

        let conn = db.conn.lock().unwrap();
        // Clear the shadow created at insert so a re-shadow is detectable
        // unambiguously: without the guard the UPDATE trigger would DELETE then
        // INSERT, leaving count = 1; with the guard it stays 0. (A plain
        // before==after check could not distinguish the two — the DELETE+INSERT
        // also nets to 1.)
        ...
        assert_eq!(
            after, 0,
            "the WHEN guard must block re-shadowing a tombstoned row (would be 1 if the trigger fired)"
        );
    }
}
