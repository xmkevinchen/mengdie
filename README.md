...
That's it. Seven tools appear in Claude Code:

| Tool | What it does |
|------|-------------|
| `memory_search` | Search memories by semantic + keyword similarity |
| `memory_ingest` | Store a new memory with metadata and entity tags |
| `memory_invalidate` | Mark a memory as no longer valid |
| `memory_get` | Fetch a full memory by id (bumps its recall count) |
| `memory_status` | DB health snapshot — counts, last ingest, metrics |
| `memory_lint` | Health checks: orphan GC, unresolved contradictions, embedding drift |
| `memory_entity_facts` | List facts tagged with a given entity |

## Usage

### In Claude Code (automatic)

Once registered, Claude Code can call `memory_search` / `memory_ingest` / `memory_invalidate` (and `memory_get` / `memory_status` / `memory_lint` / `memory_entity_facts`) in any conversation. AI tools that support MCP (like the [AE plugin](https://github.com/xmkevinchen/agentic-engineering)) will automatically use Mengdie if available.
...
- Tracks every `memory_search` call (recall count + relevance score)
...
Claude: [calls memory_ingest with structured metadata]
```
