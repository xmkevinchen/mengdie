...
1. **Independent ops cleanup** (no version tag; small commits to main):
   - One-line CLAUDE.md fix removing stale "Next step (current): residuals reduction" line.
   - Resolve production orphon `529d3212-...` (delete or restore links).
   - Run v5 migration on `~/.mengdie/db.sqlite`.
   - Verify migration completed via `PRAGMA user_version` query.
