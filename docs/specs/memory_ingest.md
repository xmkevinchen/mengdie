...
**`conflict_scan_degraded`** (F-016) — `true` when the conflict scan could
not complete (DB/index error, or a per-entity lookup was skipped). An empty
`conflicts` list with `conflict_scan_degraded: true` means "could not
determine," NOT "no conflicts" — a true duplicate may be unsurfaced. Treat as
a signal to retry or fall back to a manual `memory_search`.

**`unmatched_resolves`** (F-016) — the subset of the requested `resolves` IDs
that matched no live row (typo, cross-project, already-invalidated, or a
self-supersession guard hit). The new memory is **still inserted**; only the
old→new supersession edges for these IDs were not created. Empty list = every
requested resolve matched.
...
- `resolves=[id1, id2, ...]` → insert new + set `superseded_by=new_id` on each listed ID + invalidate them.
- If an ID in `resolves` matches no live row (doesn't exist / already invalidated / cross-project / self-supersession), the insert is **NOT** rolled back: the new memory is still created, and the unmatched ID is reported in `unmatched_resolves` (F-016) so the caller can detect the dropped edge. Only the supersession edges that did match are written.
